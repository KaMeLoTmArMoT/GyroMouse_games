...
const WEIGHTS: Record<AIPersonality, WeightVector> = {
  aggressive: { attack: 2.0, selfRisk: 0.1, cover: 0.1, crates: 0.3, chain: 1.5 },
...
export class WormAI {
  ...
  public static rollPersonality(difficulty: AIDifficulty): AIPersonality {
    ...
    const pool: AIPersonality[] = ['aggressive', 'sniper', 'looter', 'chaotic', 'default'];
  ...
  public static evaluateTurn(
    ...
    const numAngles = difficulty === 'easy' ? 3 : difficulty === 'normal' ? 5 : 7;
    ...
    const topCount = difficulty === 'easy' ? 1 : difficulty === 'normal' ? 2 : 3;
      ...
      for (const wid of allWeaponIds) {
        ...
        for (const shot of shots) {
          const score =
            ...
            + this.difficultyNoise(difficulty);
    ...
    const finalAngle = Math.max(-175, Math.min(-5, bestPlan.shot.angle + angleNoise));
  ...
  private static getCandidatePositions(
    aiWorm: Worm, terrain: TerrainManager, mapObjects: MapObject[], enemies: Worm[]
  ): { x: number; y: number }[] {
    ...
    for (const dx of [80, 160, -80, -160]) {
      ...
      if (groundY !== null) {
        ...
        if (!terrain.isSolidAt(nx, feetY - 8) && !terrain.isSolidAt(nx, feetY - 20)) {
    ...
    if (aiWorm.health < aiWorm.maxHealth * 0.6) {
      const crates = mapObjects.filter((o) => o.type === 'health_crate' && !o.isDestroyed);
      if (crates.length > 0) {
  ...
  private static evaluateCover(
    x: number, y: number, terrain: TerrainManager, enemies: Worm[]
  ): number {
    ...
    for (let i = 0; i < numRays; i++) {
      ...
      for (let s = 1; s <= steps; s++) {
        ...
        if (terrain.isSolidAt(px, py)) { blocked = true; break; }
      }
      if (blocked) score += 1;
    }
  ...
  private static evaluateCrates(x: number, y: number, mapObjects: MapObject[]): number {
    let score = 0;
  ...
  private static simulateShot(
    ...
    angleDeg: number, power: number, weaponId: WeaponId,
    windX: number, terrain: TerrainManager,
    enemies: Worm[], allies: Worm[], mapObjects: MapObject[]
  ): ShotResult {
    ...
    let enemyDamage = 0, selfDamage = 0, chainBonus = 0;
    ...
    return { angle: angleDeg, power, weaponId, enemyDamage, selfDamage, chainBonus, terrainDestruction };
  }
  ...
  private static simulateWeapon(
    originX: number, originY: number, weaponId: WeaponId,
    enemies: Worm[], allies: Worm[], terrain: TerrainManager,
    mapObjects: MapObject[], windX: number, numAngles: number
  ): ShotResult[] {
    ...
    if (enemies.length > 0) {
      ...
      const denom = targetDist * Math.sin(2 * rad) - 2 * dy * Math.cos(rad) * Math.cos(rad);
      if (denom > 0) {
        const requiredSpeed = Math.sqrt((0.45 * targetDist * targetDist) / denom);
    ...
    for (let i = 0; i < numAngles; i++) {
      ...
      for (const pDelta of [-0.1, 0, 0.1]) {
        ...
        const shot = this.simulateShot(
          originX, originY, clampedAngle, power, weaponId,
          windX, terrain, enemies, allies, mapObjects
        );
        results.push(shot);
      }
    }
