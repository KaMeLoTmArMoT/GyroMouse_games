import { WeaponId, CELL_ACID } from '../types';
...
export class Projectile {
  ...
  constructor(
    ...
    if (weaponId === 'grenade' || weaponId === 'cluster' || weaponId === 'dynamite') {
      this.bounciness = 0.6;
    }
  ...
  public update(
    ...
    if (this.weaponId === 'bazooka' || this.weaponId === 'cluster' || this.weaponId === 'drill' || this.weaponId === 'mortar') {
      this.vx += windX * 0.05;
    }
    ...
    const hasFuse = this.weaponId === 'grenade'
    ...
    for (const obj of mapObjects) {
      ...
      if (dist < this.radius + obj.radius) {
        if (this.weaponId === 'grenade' || this.weaponId === 'cluster' || this.weaponId === 'dynamite') {
    ...
    if (isSolidNow || isSolidNext) {
      if (this.weaponId === 'grenade' || (this.weaponId === 'cluster' && !this.isClusterChild) || this.weaponId === 'dynamite') {
        // Bounce off terrain
        ...
        if (!this.drillPenetrated) {
          ...
          for (let d = 0; d < tunnelLen; d += 3) {
            ...
            for (let w = -Math.ceil(tunnelWidth / terrain.cellScale); w <= Math.ceil(tunnelWidth / terrain.cellScale); w++) {
  ...
  public triggerExplosion(
    ...
    if (this.weaponId === 'sand_bomb') {
      ...
      const blastRadius = this.weaponId === 'bazooka' ? 42 : this.weaponId === 'cluster' ? 30 : this.weaponId === 'drill' ? 35 : 38;
      const baseDamage = this.weaponId === 'bazooka' ? 45 : this.weaponId === 'drill' ? 40 : 35;
