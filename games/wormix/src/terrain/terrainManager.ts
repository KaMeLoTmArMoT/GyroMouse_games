...
export class TerrainManager {
	...
	constructor(width: number, height: number) {
		...
		this.gridImageData = this.offscreenCtx.createImageData(
			this.gridWidth,
			this.gridHeight,
		);
	...
	public getCell(gx: number, gy: number): number {
		if (gx < 0 || gx >= this.gridWidth || gy < 0 || gy >= this.gridHeight) {
			return gy >= Math.floor(this.waterY / this.cellScale)
				? CELL_BEDROCK
				: CELL_AIR;
		}
	...
	public generateTerrain(): void {
		...
		for (let gx = 0; gx < this.gridWidth; gx++) {
			...
			let groundY = Math.min(
				this.waterY - 70,
				Math.max(120, this.height * 0.62 + heightOffset),
			);
			if (gx >= sandStartG && gx <= sandEndG) {
				const sandPeak =
					Math.sin(((gx - sandStartG) / (sandEndG - sandStartG)) * Math.PI) *
					25;
				groundY -= sandPeak;
			}
		...
		for (let gx = lakeStartG; gx <= lakeEndG; gx++) {
			const surfaceGY = Math.floor(
				this.getSurfaceY(gx * this.cellScale) / this.cellScale,
			);
	...
	public buildTerrainFromHeights(
		...
		for (let gx = 0; gx < this.gridWidth; gx++) {
			const xWorld = gx * this.cellScale;
			const groundY =
				customHeights[xWorld] !== undefined
					? customHeights[xWorld]
					: this.height * 0.65;
			const groundGY = Math.floor(groundY / this.cellScale);
	...
	public getLocalGroundY(
		...
		const maxDownGY = Math.min(
			this.gridHeight - 1,
			startGY + Math.ceil(maxSearchDown / this.cellScale),
		);
		const maxUpGY = Math.max(
			0,
			startGY - Math.ceil(maxSearchUp / this.cellScale),
		);
	...
	public getWaterDensityAt(
		x: number,
		y: number,
		radiusWorld: number = 24,
	): number {
	...
	public spawnElementStream(
		xWorld: number,
		yWorld: number,
		cellType: number,
		radiusWorld: number = 15,
	): void {
				...
				for (let gx = xStart; gx !== xEnd; gx += xStep) {
					...
					if (cell === CELL_ACID && pass === 0) {
						...
						for (const nIdx of neighbors) {
							...
							if (
								targetMat === CELL_GRASS ||
								targetMat === CELL_DIRT ||
								targetMat === CELL_STONE ||
								targetMat === CELL_SAND
							) {
					...
					if (cellBelow === CELL_AIR) {
						...
						if (
							diag1GX >= 0 &&
							diag1GX < this.gridWidth &&
							updateGrid[(gy + 1) * this.gridWidth + diag1GX] === CELL_AIR
						) {
							...
							diag2GX >= 0 &&
							diag2GX < this.gridWidth &&
							updateGrid[(gy + 1) * this.gridWidth + diag2GX] === CELL_AIR
						) {
							...
							for (const dir of [checkDir, -checkDir]) {
								...
								if (
									hGX >= 0 &&
									hGX < this.gridWidth &&
									updateGrid[gy * this.gridWidth + hGX] === CELL_AIR
								) {
			...
			for (let gx = xStart; gx !== xEnd; gx += xStep) {
				...
				if (cellBelow === CELL_AIR) {
					...
					if (
						diag1GX >= 0 &&
						diag1GX < this.gridWidth &&
						updateGridSand[(gy + 1) * this.gridWidth + diag1GX] === CELL_AIR
					) {
						const diag1Idx = (gy + 1) * this.gridWidth + diag1GX;
						this.grid[idx] = CELL_AIR;
						...
						diag2GX >= 0 &&
						diag2GX < this.gridWidth &&
						updateGridSand[(gy + 1) * this.gridWidth + diag2GX] === CELL_AIR
					) {
						const diag2Idx = (gy + 1) * this.gridWidth + diag2GX;
						this.grid[idx] = CELL_AIR;
	...
	public draw(ctx: CanvasRenderingContext2D): void {
		...
		for (let gx = 0; gx < this.gridWidth; gx++) {
			...
			const leftY =
				gx > 0 && rawTopY[gx - 1] >= 0 ? rawTopY[gx - 1] : rawTopY[gx];
			const rightY =
				gx < this.gridWidth - 1 && rawTopY[gx + 1] >= 0
					? rawTopY[gx + 1]
					: rawTopY[gx];
			if (
				Math.abs(leftY - rawTopY[gx]) <= 6 &&
				Math.abs(rightY - rawTopY[gx]) <= 6
			) {
				validSurface[gx] = 1;
			}
		}
