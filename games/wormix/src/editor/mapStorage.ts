...
export class MapStorage {
  public static getPresetMaps(width: number, height: number): CustomMapData[] {
    ...
    for (let x = 0; x < width; x++) {
      if ((x > width * 0.15 && x < width * 0.35) || (x > width * 0.65 && x < width * 0.85)) {
        archipelagoHeights[x] = baseGroundY - Math.sin(((x % (width * 0.2)) / (width * 0.2)) * Math.PI) * 80;
      }
    }
    ...
    for (let x = 0; x < width; x++) {
      ...
      if ((x > width * 0.1 && x < width * 0.3) || (x > width * 0.7 && x < width * 0.9)) {
        h -= 90; // Fort towers
      }
      fortsHeights[x] = h;
    }
