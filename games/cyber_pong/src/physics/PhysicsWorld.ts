...
export class PhysicsWorld {
	...
	private setupArenaColliders() {
		...
		const topWallBody = this.world.createRigidBody(
			RAPIER.RigidBodyDesc.fixed().setTranslation(0, 0.5, -10.3),
		);
		this.world
			.createCollider(topWallGeo, topWallBody)
			.setRestitution(wallMaterial.restitution);

		const botWallGeo = RAPIER.ColliderDesc.cuboid(18.4, 1, 0.3);
		const botWallBody = this.world.createRigidBody(
			RAPIER.RigidBodyDesc.fixed().setTranslation(0, 0.5, 10.3),
		);
		this.world
			.createCollider(botWallGeo, botWallBody)
			.setRestitution(wallMaterial.restitution);
		...
		const p1TopFlankBody = this.world.createRigidBody(
			RAPIER.RigidBodyDesc.fixed().setTranslation(-18.3, 0.5, -7),
		);
		this.world
			.createCollider(flankGeo, p1TopFlankBody)
			.setRestitution(wallMaterial.restitution);

		const p1BotFlankBody = this.world.createRigidBody(
			RAPIER.RigidBodyDesc.fixed().setTranslation(-18.3, 0.5, 7),
		);
		this.world
			.createCollider(flankGeo, p1BotFlankBody)
			.setRestitution(wallMaterial.restitution);

		const p2TopFlankBody = this.world.createRigidBody(
			RAPIER.RigidBodyDesc.fixed().setTranslation(18.3, 0.5, -7),
		);
		this.world
			.createCollider(flankGeo, p2TopFlankBody)
			.setRestitution(wallMaterial.restitution);

		const p2BotFlankBody = this.world.createRigidBody(
			RAPIER.RigidBodyDesc.fixed().setTranslation(18.3, 0.5, 7),
		);
		this.world
			.createCollider(flankGeo, p2BotFlankBody)
			.setRestitution(wallMaterial.restitution);
	}
	...
	private setupPaddles() {
		...
		const p1Desc = RAPIER.RigidBodyDesc.kinematicPositionBased().setTranslation(
			-14,
			0.4,
			0,
		);
		this.p1PaddleBody = this.world.createRigidBody(p1Desc);
		const paddleCol1 = RAPIER.ColliderDesc.cuboid(0.25, 0.4, 1.6)
			.setRestitution(1.2)
			.setFriction(0.0);
		this.p1Collider = this.world.createCollider(paddleCol1, this.p1PaddleBody);

		const p2Desc = RAPIER.RigidBodyDesc.kinematicPositionBased().setTranslation(
			14,
			0.4,
			0,
		);
		this.p2PaddleBody = this.world.createRigidBody(p2Desc);
		const paddleCol2 = RAPIER.ColliderDesc.cuboid(0.25, 0.4, 1.6)
			.setRestitution(1.2)
			.setFriction(0.0);
	...
	public servePuck(direction: "left" | "right", speedMultiplier = 1.0) {
		...
		this.puckBody.setLinvel(
			{ x: dirX * baseSpeed, y: 0, z: dirZ * baseSpeed },
			true,
		);
	}
	...
	public step(dt: number, isP1AllCleared: boolean, isP2AllCleared: boolean) {
		...
		const inEnemy =
			(this.puckOwner === "p1" && puckPos.x > 0) ||
			(this.puckOwner === "p2" && puckPos.x < 0);
		...
		const minSpeed =
			(isP1AllCleared || isP2AllCleared ? 16.8 : 11.2) * totalMultiplier;
		const maxSpeed =
			(isP1AllCleared || isP2AllCleared ? 25.2 : 18.2) * totalMultiplier;
			...
			if (handle === this.p1Collider.handle) {
				...
				for (const [id, brick] of this.bricks.entries()) {
					if (brick.collider.handle === handle) {
						isBrick = true;
						if (
							(this.puckOwner === "p1" && brick.side === "p1") ||
		...
		if (hitPaddleThisFrame && now - this.lastPaddleHitTime > 150) {
			...
			const paddleBody =
				hitPaddle === "p1" ? this.p1PaddleBody : this.p2PaddleBody;
		...
		for (const id of destroyedBrickIds) {
			...
			if (brick) {
				...
				if (side === "p1") {
					...
					this.puckBody.setLinvel(
						{ x: -Math.abs(currentVel.x || 12), y: 0, z: currentVel.z },
						true,
					);
				} else {
