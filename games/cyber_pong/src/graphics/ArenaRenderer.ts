...
export class ArenaRenderer {
	...
	private setupArenaVisuals() {
		...
		const topWall = new THREE.Mesh(
			new THREE.BoxGeometry(36.8, 1.2, 0.6),
			wallMat,
		);
		...
		const p1GoalMat = new THREE.MeshStandardMaterial({
			color: 0x0284c7,
			emissive: 0x0284c7,
		...
		const p2GoalMat = new THREE.MeshStandardMaterial({
			color: 0xe11d48,
			emissive: 0xe11d48,
			emissiveIntensity: 0.8,
			transparent: true,
	...
	public updatePuckTrail(puckPos: THREE.Vector3) {
		...
		this.puckTrailGeometry.setAttribute(
			"position",
