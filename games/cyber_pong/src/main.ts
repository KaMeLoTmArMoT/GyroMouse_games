...
import { AIBotController, type AIDifficulty } from "./ai/AIBotController";
...
export class CyberPongGame extends BaseGame {
	...
	private initUI() {
		...
		this.modeSelectEl = document.getElementById(
			"mode-select",
		) as HTMLSelectElement;
		this.diffSelectEl = document.getElementById(
			"diff-select",
		) as HTMLSelectElement;
		...
		this.btnRestartEl = document.getElementById(
			"btn-restart",
		...
		const updateControlsHint = () => {
			...
			if (this.mode === "2p") {
				if (p1HintEl)
					p1HintEl.innerHTML =
						'P1 (Blue): <span class="key-badge">W</span>/<span class="key-badge">S</span> or <span class="key-badge">↑</span>/<span class="key-badge">↓</span>';
				if (p2HintEl) {
					p2HintEl.innerHTML =
						'P2 (Red): <span class="key-badge">A</span>/<span class="key-badge">D</span> or <span class="key-badge">←</span>/<span class="key-badge">→</span>';
		...
		this.diffSelectEl.style.display =
			this.mode === "1p" ? "inline-block" : "none";
		updateControlsHint();
	...
	private onBrickDestroyed(id: string, side: "p1" | "p2") {
		...
		if (side === "p1") {
			...
			this.p1Scale =
				1.0 + (this.p1BricksCleared / this.totalBricksPerSide) * 0.8;
			...
			this.p2Scale =
				1.0 + (this.p2BricksCleared / this.totalBricksPerSide) * 0.8;
	...
	private gameLoop(time: number) {
		...
		if (this.isStarted && !this.isPaused && !this.isGameOver) {
			...
			this.renderer.updatePuckTrail(
				new THREE.Vector3(puckPos.x, puckPos.y, puckPos.z),
			);
		}
