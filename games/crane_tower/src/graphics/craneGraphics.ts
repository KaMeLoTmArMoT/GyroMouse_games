...
export class CraneGraphicsManager {
  ...
  private buildEnvironment() {
    ...
    for (let x = -20; x <= 20; x += 0.8) {
      ...
      const tieMat = new THREE.MeshStandardMaterial({ color: '#334155', roughness: 0.9 });
  ...
  private buildGantryCrane() {
    ...
    const trolleyMat = new THREE.MeshStandardMaterial({ color: '#0284c7', metalness: 0.8 });
    ...
    const magMat = new THREE.MeshStandardMaterial({ color: '#1e293b', metalness: 0.9, roughness: 0.2 });
    ...
    const cableMat = new THREE.LineBasicMaterial({ color: '#94a3b8', linewidth: 2 });
  ...
  private buildTrainPlatform() {
    ...
    const bedMat = new THREE.MeshStandardMaterial({ color: '#dc2626', metalness: 0.5, roughness: 0.4 });
    ...
    const wheelMat = new THREE.MeshStandardMaterial({ color: '#1e293b', metalness: 0.9, roughness: 0.2 });
   ...
   */
  public addCrateMesh(id: string, size: { x: number; y: number; z: number }, colorHex = '#0284c7'): THREE.Mesh {
  ...
  public syncGraphics(physics: CranePhysicsManager) {
    ...
    const positions = this.cableLine.geometry.attributes.position as THREE.BufferAttribute;
