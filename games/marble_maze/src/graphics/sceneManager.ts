...
export class SceneManager {
	...
	constructor(container: HTMLElement) {
		...
		for (let i = 0; i < 4; i++) {
			...
			light.position.set(
				cornerPositions[i].x * 25,
				40,
				cornerPositions[i].z * 25,
			);
	...
	public buildMazeMesh(maze: MazeData, debugPathEnabled: boolean = false) {
		console.log(
			`[SCENE DEBUG] Building maze mesh for ${maze.width}x${maze.height} maze`,
		);
				...
				if (cell.isBridge) {
					...
					if (isZ) {
						...
						for (const h of holes) {
							const pitGeo = new THREE.BoxGeometry(
								h.halfW * 1.8,
								0.45,
								cellSize * 0.9,
							);
					...
					if (isMoving) {
						...
						if (isSquare) {
							...
							const pitRingGeo = new THREE.TorusGeometry(
								halfExtent,
								0.05,
								16,
								32,
			...
			for (let x = 0; x < maze.width; x++) {
				...
				if (cell.isGate) {
					...
					if (currentIdx === undefined) {
						console.warn(
							`Gate at (${x},${z}) not on main path - this shouldn't happen`,
						);
					}
		...
		// Debug summary
		console.log(
			`[SCENE DEBUG] Rendered: ${floorTiles} floor tiles, ${guardrailSegments} guardrail segments, ${wallSegments} wall segments`,
		);
		console.log(
			`[SCENE DEBUG] Elements: ${coinsRendered} coins, ${gatesRendered} gates, ${holesRendered} holes, ${bridgesRendered} bridges`,
		);
	...
	public setCheckpointActiveState(
		checkpointId: string,
		state: "inactive" | "active" | "claimed",
	) {
