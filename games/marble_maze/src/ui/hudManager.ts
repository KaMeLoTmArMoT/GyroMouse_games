...
export class HudManager {
	...
	private bindDOM() {
		...
		document
			...
			?.addEventListener("click", () => this.callbacks.onNewRandom());
		document.getElementById("btn-mute")?.addEventListener("click", (e) => {
			const isMuted = this.callbacks.onToggleMute();
			(e.currentTarget as HTMLElement).innerText = isMuted
				? "🔇 Muted"
				: "🔊 Audio";
		});
	...
	public updateDebugPathSetting(enabled: boolean) {
		const debugPathCheckbox = document.getElementById(
			"setting-debug-path",
		) as HTMLInputElement;
	...
	public showFallModal(hasCheckpoint: boolean) {
		...
		const restartFromCheckpointBtn = document.getElementById(
			"btn-fall-checkpoint",
		);
		...
		if (restartFromCheckpointBtn) {
			if (hasCheckpoint) {
				...
				restartFromCheckpointBtn.onclick = () => {
					this.closeFallModal();
					(this.callbacks.onRestart as (fromCheckpoint?: boolean) => void)(
						true,
					);
				};
			} else {
