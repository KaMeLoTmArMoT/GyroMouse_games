...
export class PhysicsManager {
	...
	public gateSensors: Map<
		string,
		{ sensor: RAPIER.Collider; blocker: RAPIER.Collider; cooldown: number }
	> = new Map();
	public checkpointSensors: Map<
		string,
		{ sensor: RAPIER.Collider; position: { x: number; y: number; z: number } }
	> = new Map();
			...
			for (let x = 0; x < maze.width; x++) {
				...
				if (cell.isBridge) {
					...
					if (isZ) {
						...
						const bDesc = RAPIER.ColliderDesc.cuboid(
							halfBridge * 0.9,
							0.4,
							holeZ,
						)
						...
						for (const h of holes) {
							const sDesc = RAPIER.ColliderDesc.cuboid(
								h.halfW * 0.9,
								0.2,
								holeZ,
							)
								...
								.setSensor(true)
								.setActiveEvents(RAPIER.ActiveEvents.COLLISION_EVENTS);
							this.holeSensors.push(
								this.world.createCollider(sDesc, boardBody),
							);
						}
					} else {
					...
					const defaultCfg = {
						shape: "round" as HoleShape,
						radius: 0.5,
						size: 0,
						offsetX: 0,
						offsetZ: 0,
						movePattern: "static" as HoleMovePattern,
						moveSpeed: 0,
						moveRange: 0,
					};
					...
					if (isMoving) {
						...
						const leftSlabDesc = RAPIER.ColliderDesc.cuboid(
							leftW,
							0.4,
							halfExtent,
						)
						...
						const rightSlabDesc = RAPIER.ColliderDesc.cuboid(
							rightW,
							0.4,
							halfExtent,
						)
							...
							.setFriction(friction)
							.setRestitution(restitution);
						const rightSlab = this.world.createCollider(
							rightSlabDesc,
							boardBody,
						);
						...
						if (isSquare) {
							sensorDesc = RAPIER.ColliderDesc.cuboid(
								halfExtent * 0.9,
								0.2,
								halfExtent * 0.9,
							).setTranslation(holeWorldX, -0.2, holeWorldZ);
						} else {
							sensorDesc = RAPIER.ColliderDesc.cylinder(
								0.2,
								halfExtent * 0.9,
							).setTranslation(holeWorldX, -0.2, holeWorldZ);
						}
						...
						if (isSquare) {
							...
							holeSensorDesc = RAPIER.ColliderDesc.cuboid(
								s,
								0.2,
								s,
							).setTranslation(holeWorldX, -0.2, holeWorldZ);
						} else {
							holeSensorDesc = RAPIER.ColliderDesc.cylinder(
								0.2,
								halfExtent * 0.9,
							).setTranslation(holeWorldX, -0.2, holeWorldZ);
						}
						...
						const topSlabDesc = RAPIER.ColliderDesc.cuboid(
							halfCell,
							0.4,
							topRimH / 2,
						)
							.setTranslation(
								cellCenterX,
								-0.4,
								cellCenterZ - halfCell + topRimH / 2,
							)
							.setFriction(friction)
							.setRestitution(restitution);
						this.world.createCollider(topSlabDesc, boardBody);

						const botSlabDesc = RAPIER.ColliderDesc.cuboid(
							halfCell,
							0.4,
							botRimH / 2,
						)
							.setTranslation(
								cellCenterX,
								-0.4,
								cellCenterZ + halfCell - botRimH / 2,
							)
							.setFriction(friction)
							.setRestitution(restitution);
						...
						const leftSlabDesc = RAPIER.ColliderDesc.cuboid(
							...
							.setTranslation(
								cellCenterX - halfCell + leftRimW / 2,
								-0.4,
								holeWorldZ,
							)
							.setFriction(friction)
						...
						const rightSlabDesc = RAPIER.ColliderDesc.cuboid(
							...
							.setTranslation(
								cellCenterX + halfCell - rightRimW / 2,
								-0.4,
								holeWorldZ,
							)
							.setFriction(friction)
				...
				if (cell.hasCoin && !cell.isHole && !cell.isBridge) {
					...
					const coinSensor = this.world.createCollider(
						coinSensorDesc,
						boardBody,
					);
				...
				if (cell.isGoal) {
					const goalDesc = RAPIER.ColliderDesc.cuboid(
						halfCell * 0.8,
						0.5,
						halfCell * 0.8,
					)
					...
					const blockerDesc =
						blockDir === "top" || blockDir === "bottom"
							...
							: RAPIER.ColliderDesc.cuboid(
									...
									cellCenterX + (blockDir === "left" ? -halfCell : halfCell),
									wallHalfHeight,
									cellCenterZ,
								);
	...
	private isMarbleInsideGate(gate: {
		sensor: RAPIER.Collider;
		blocker: RAPIER.Collider;
		cooldown: number;
	}): boolean {
	...
	public updateMovingHoles(dt: number) {
		for (const mh of this.movingHoles) {
			...
			const topDesc = RAPIER.ColliderDesc.cuboid(hC, 0.4, topD)
				...
				.setFriction(mh.friction)
				.setRestitution(mh.restitution);
	...
	public step(dt: number): {
		...
		if (this.callbacks) {
			...
			if (
				this.goalSensor &&
				this.world.intersectionPair(this.marbleCollider, this.goalSensor)
			) {
			...
			this.checkpointSensors.forEach((checkpoint, checkpointId) => {
				if (
					this.world.intersectionPair(this.marbleCollider, checkpoint.sensor)
				) {
