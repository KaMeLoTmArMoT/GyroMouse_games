...
export class MazeGenerator {
	public static generate(
		...
		for (let z = 0; z < height; z++) {
			...
			for (let x = 0; x < width; x++) {
				cells[z][x] = {
					...
					terrain: MazeGenerator.getDefaultTerrainForTheme(theme),
					isHole: false,
					isBridge: false,
					isGoal: false,
					isStart: false,
		...
		while (stack.length > 0) {
			...
			const neighbors = MazeGenerator.getUnvisitedNeighbors(
				current.x,
				current.z,
				width,
				height,
			...
			if (neighbors.length > 0) {
				...
				MazeGenerator.removeWallsBetween(
					cells[current.z][current.x],
					cells[next.z][next.x],
				);
		...
		const mainPath = MazeGenerator.findMainPath(cells, width, height);
		const pathIndex = new Map<string, number>();
		mainPath.forEach((p, i) => pathIndex.set(`${p.x},${p.z}`, i));

		MazeGenerator.assignOrganicTerrains(
			cells,
			width,
			height,
			theme,
		...
		let coinsCount = 0;
		const holeProb =
			difficulty === "easy" ? 0.05 : difficulty === "medium" ? 0.1 : 0.18;
		...
		for (let z = 0; z < height; z++) {
			for (let x = 0; x < width; x++) {
				...
				if (openWallsCount === 1) {
					...
					cell.isHole = true;
					cell.holeConfig = MazeGenerator.generateHoleConfig(prng);
					holesCount++;
		...
		for (let i = 0; i < gateCount; i++) {
			...
			for (let a = 0; a < mainPath.length && !placed; a++) {
				const candidateIdx =
					Math.floor(prng.next() * (mainPath.length - 3)) + 1;
		...
		MazeGenerator.placeBridges(cells, width, height, difficulty, prng);
		...
		console.log(
			`[MAZE DEBUG] Generated maze ${width}x${height}, seed: ${seedStr}, difficulty: ${difficulty}`,
		);
		console.log(`[MAZE DEBUG] Main path length: ${mainPath.length}`);
		console.log(
			`[MAZE DEBUG] Elements: ${holesCount} holes, ${coinsCount} coins, ${gatesCount} gates, ${checkpointsCount} checkpoints`,
		);
		...
		console.log(
			`[MAZE DEBUG] Checkpoints at: ${checkpointLocs.join(", ") || "none"}`,
		);
			...
			for (let x = 0; x < width; x++) {
				if (cells[z][x].isHole) {
					...
					if (config?.shape === "round") {
						roundHoles.push(
							`(${x},${z})${config.movePattern !== "static" ? " moving" : ""}`,
						);
					} else {
						squareHoles.push(
							`(${x},${z})${config?.movePattern !== "static" ? " moving" : ""}`,
					...
					if (config?.movePattern !== "static") {
						movingHoles.push(
							`(${x},${z}) ${config?.shape || "unknown"} ${config?.movePattern || "unknown"}`,
						);
					}
				}
			}
		...
		console.log(
			`[MAZE DEBUG] Square holes: ${squareHoles.join(", ") || "none"}`,
		);
		console.log(
			`[MAZE DEBUG] Moving holes: ${movingHoles.join(", ") || "none"}`,
		);
			...
			for (let x = 0; x < width; x++) {
				if (cells[z][x].isBridge) {
					bridgePositions.push(
						`(${x},${z}) ${cells[z][x].bridgeConfig?.lane || "unknown"}`,
					);
				}
			}
		}
		console.log(
			`[MAZE DEBUG] Bridges at: ${bridgePositions.join(", ") || "none"}`,
		);
		...
		console.log(
			`[MAZE DEBUG] Walls: ${totalWalls} total, ${avgWallsPerCell} avg per cell, ${cellsWithWalls}/${width * height} cells have walls`,
		);
	...
	private static generateHoleConfig(prng: SeededRandom): HoleConfig {
		...
		const movePatterns: HoleMovePattern[] = [
			"static",
			"static",
			"static",
			"horizontal",
	...
	private static placeBridges(
		...
		const bridgeProb = difficulty === "medium" ? 0.05 : 0.1;
	...
	private static getUnvisitedNeighbors(
		...
		if (x < width - 1 && !cells[z][x + 1].visited)
			neighbors.push({ x: x + 1, z });
		if (z < height - 1 && !cells[z + 1][x].visited)
	...
	private static assignOrganicTerrains(
		...
		const terrainPalette = MazeGenerator.getTerrainPalette(theme);
		const zoneCount = Math.min(
			terrainPalette.length,
			Math.max(2, Math.floor(mainPath.length / 3)),
		);
		...
		const assignedTerrains: TerrainType[] = [];
		...
		for (let i = assignedTerrains.length - 1; i > 0; i--) {
			...
			[assignedTerrains[i], assignedTerrains[j]] = [
				assignedTerrains[j],
				assignedTerrains[i],
			];
		}
		...
		for (let z = 0; z < height; z++) {
			for (let x = 0; x < width; x++) {
				...
				if (cell.isStart || cell.isGoal) {
					cell.terrain = MazeGenerator.getDefaultTerrainForTheme(theme);
					continue;
				}
