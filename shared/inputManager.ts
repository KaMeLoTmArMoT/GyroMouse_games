...
export class SharedInputManager {
  ...
  public getSteeringValue(options?: SteeringOptions): { x: number; y: number } {
    ...
    const sensitivity = options?.sensitivity ?? this.settings.sensitivity ?? 1.0;
    ...
    if (this.hasGyroData) {
      ...
      // Keyboard fallback
      if (this.keysPressed.has('ArrowRight') || this.keysPressed.has('KeyD')) roll += 1.0;
      if (this.keysPressed.has('ArrowLeft') || this.keysPressed.has('KeyA')) roll -= 1.0;
      if (this.keysPressed.has('ArrowDown') || this.keysPressed.has('KeyS')) pitch += 1.0;
      if (this.keysPressed.has('ArrowUp') || this.keysPressed.has('KeyW')) pitch -= 1.0;
    }
    ...
    const x = Math.max(-1, Math.min(1, scaledRoll * sensitivity * (invertX ? -1 : 1)));
    const y = Math.max(-1, Math.min(1, scaledPitch * sensitivity * (invertY ? -1 : 1)));
  ...
  public update(dt: number) {
    ...
    if (this.hasGyroData) {
      ...
      rawDx = steering.x;
      rawDy = steering.y;
    } else if (this.settings.mouseEnabled && (this.settings.mode === 'gyromouse' || this.settings.mode === 'both')) {
    ...
    if (!this.hasGyroData && (this.settings.mode === 'keyboard' || this.settings.mode === 'both')) {
